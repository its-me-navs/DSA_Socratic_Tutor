...
3. routes/chats
    send_message
        session holds the qeury result, the first result that matches the session id and user id
        the message history holds query result of all messages in that particular session
        history appends each message according to role, and also appends the current prompt
        all embeddings are saved in embeddings
        te current prompt is sent to get_embeddings which converts the text to a dense vector embedding of 384 dimensions, and returns a json vector, which is then loaded back as a python object
        then the cosine similarity between the query_vec and each embedding in all_embeddings is stored as an array in scored
        then scored is sorted in revsers eorder to get the maximum similarity first, thats why x[0]
        the top score ad top item will be the first elemnts in the sorted scored
        then check if the similarity is greater than .4, if not, its not that similar and theres no use injectig it into the prompt, so only if relevant inject it into the context snippet 
        the final system prompt will contain this context snipper 
        a respone is generated by lllama and the messages sent to it will contain the system prompt +history
        the new messages (aka the current message by user, and the response by the llm) is then added into the db and the response by the llm is returned by this function
4. routes/review
    add_review
        the next_review is made 'now' so that it immediately appears in the review list
    due_reviews
        shows the id, problem, stage and next review of all problems due today or is overdue
    complete_review 
        increments the next_review (if its lesser than the number of reviews left) from now using the review_intervals
    we use refresh(item) during add_review coz a new object is created by the db, in order to get the value back, we refresh, however, when modifying an existing object (got the object after querying) python already knows the value coz we fed it, so no refresh needed

5. routes/rag
        add_embedding
            the content of the body is passed through get_embedding which (dumps) returns the json object equivalent of the vector embedding
            this is then added to the EmbeddingItem table, coz PostgreSQL can only store strings...so converted to json string
        search_embedding
            a qeury vector is created which is the json object returned from get-embedding, which is then (loads) converted back to python list of vector embedding
            all the embeddings of the user is queried, and stored in items
            each item in items is then loaded, and cosine similairty is performed on it and the qeury_vec, and the score is appended to scored
            the scored is sorted in descending order and the top_k (3) are selected based on their score 
        JSON serialization pattern: embeddings are stored as JSON strings (PostgreSQL can't store raw arrays without pgvector). When storing → keep as JSON string. When computing → json.loads() back to Python list first.